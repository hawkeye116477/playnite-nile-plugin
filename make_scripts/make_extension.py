...
extFile = pj(mainPath, "Releases", "NileLibrary_" +
             versionUnderline + ".pext")
checksumFilePath = pj(mainPath, "Releases",
                      "NileLibrary_" + versionUnderline + ".pext.sha256")
...
if os.path.exists(extFile):
    ...
    nileProj = ET.parse(pj(mainPath, "src", "NileLibrary.csproj"))
    nileProj_root = nileProj.getroot()
    
    for child in nileProj_root.findall(".//msbuild:PackageReference", namespaces):
    ...
    if newVersion == "true":
        installerManifest["Packages"].insert(0, {
            ...
            "PackageUrl": f"https://github.com/hawkeye116477/playnite-nile-plugin/releases/download/{version}/NileLibrary_{versionUnderline}.pext",
